...
PROZESS-ARCHITEKTUR:
  p_m1  | M1 Observer     | Cores 1-3 | EMA-Tracker + S4-Gate | 1 Hz Tick-Rate
  p_m2  | M2 Tensor       | Cores 4-5 | Metrik-Aggregation    | Empfängt S4-Pass-Through
  p_m3  | M3 Decision     | Core 6    | Bayesian Engine       | SHORT/MONITOR/IGNORE
  p_ctl | Control Loop    | Core 7    | Watchdog + Restart    | 5s Health-Check

LOG-HIERARCHIE (RISK: Log Asphyxiation):
  p_m1 (Hot-Path):  Level WARNING|ERROR — Nur Ausnahmen! Kein I/O bei INFO|DEBUG!
  p_m2 (Warm/Cold): Level INFO — aggregierte Heartbeats alle 300s
  p_m3 (Warm/Cold): Level INFO — aggregierte Heartbeats alle 300s
  p_ctl (Watchdog):  Level INFO — Heartbeat alle 5s auf Disk

Core 0 ist RESERVIERT für Linux-Kernel + NIC. Kein User-Space erlaubt.
...
logging.basicConfig(
    level=logging.WARNING,   # RISK: Log Asphyxiation — p_m1 Hot-Path MUSS auf WARNING
...
def p_m1_observer(
    ...
    log.warning(f"M1 Observer gestartet | Cores={CPU_M1_RANGE} | EMA_ALPHA={EMA_ALPHA:.6f} | "
               f"OB_THRESH={OB_DEPTH_THRESHOLD} | VOL_REQ={VOLUME_SPIKE_REQ}x | Log-Level=WARNING (Hot-Path)")
    ...
    while not stop_event.is_set():
        ...
        # HOT-PATH: Nur S4-PASS (selten!) wird als WARNING geloggt.
        # ILLIQUID/UNCERTAIN-Rejects werden NICHT geloggt (Log Asphyxiation-Schutz).
        if pass_to_m2:
            try:
                m2_queue.put_nowait({
                    'tick':            tick_counter,
                ...
                # WARNING: Nur S4-PASS-Events (selten) → kein I/O-Bottleneck
                log.warning(
                ...
                pass  # Queue full — M2 zu langsam, Tick verwerfen (kein Log im Hot-Path!)

        # NOTE: ILLIQUID- und UNCERTAIN-Rejects werden hier absichtlich NICHT geloggt.
        # Jeder Log im Hot-Path ist ein potentieller I/O-Blocker (Log Asphyxiation).
        # Bei Bedarf: Aggregation via p_m2 oder p_ctl (nicht im Hot-Path).
    ...
    log.warning("M1 Observer: Stop-Signal empfangen — sauberes Shutdown.")
...
def p_m2_tensor(
    ...
    while not stop_event.is_set():
        ...
        # Aggregierter Heartbeat alle 300 Ticks (5 Minuten bei 1 Hz M1-Rate)
        if tick_heartbeat_counter >= 300:
            log.info(f"M2 Heartbeat | 300 Ticks | PASS→M3: {pass_count} | Queue-Status: OK")
            tick_heartbeat_counter = 0
            pass_count = 0
...
def p_m3_decision(
    ...
    while not stop_event.is_set():
        ...
        if action == 'IMMEDIATE_SHORT':
            short_count += 1
        else:
            monitor_count += 1
        tick_heartbeat_counter += 1

        # Aggregierter Heartbeat alle 300 Ticks
        if tick_heartbeat_counter >= 300:
            log.info(f"M3 Heartbeat | 300 Ticks | SHORT: {short_count} | MONITOR: {monitor_count}")
            tick_heartbeat_counter = 0
            short_count = 0
            monitor_count = 0
...
def p_ctl_watchdog(
    ...
    """
    Control Loop — Core 7 (Warm/Cold)
    Überwacht alle Prozesse, startet abgestürzte neu.
    Heartbeat alle 5s auf INFO (Watchdog-only, kein Hot-Path).
    Beendet alle Prozesse wenn stop_event gesetzt wird.
    """
    os.sched_setaffinity(0, {CPU_CTL})
    log = logging.getLogger('CTL_WATCHDOG')
    log.info(f"Control Loop gestartet | Core={CPU_CTL} | Interval={check_interval}s | Log-Level=INFO")
...
def main(validation_gate_status: str = 'BLOCKED') -> None:
    ...
    logger.info("Log-Hierarchie: p_m1=WARNING (Hot-Path, kein INFO I/O), p_m2/p_m3/p_ctl=INFO (aggregierte Heartbeats 300t)")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='TRINITY APEX MAIN MP')
    parser.add_argument(
        '--gate', default=os.environ.get('VALIDATION_GATE', 'BLOCKED'),
        help="Validation-Gate Status (VALIDATED|BLOCKED|DEGRADED). Default: VALIDATION_GATE env var."
    )
    parser.add_argument(
        '--dry-run', action='store_true',
        help='DRY-RUN: Keine Prozesse starten. Nur Affinitäts-Verify + Struct-Size-Check.'
    )
    args = parser.parse_args()

    dry_run = os.environ.get('TRINITY_DRY_RUN', '').lower() in ('true', '1', 'yes') or args.dry_run

    if dry_run:
        print("=" * 60)
        print("  TRINITY APEX MAIN MP — DRY-RUN MODE")
        print("=" * 60)
        print(f"  ValidationGate: {args.gate}")
        print(f"  CPU-Affinität: p_m1 Cores {CPU_M1_RANGE}, p_m2 Cores {CPU_M2_RANGE}, p_m3 Core {CPU_M3}, p_ctl Core {CPU_CTL}")
        print(f"  Core 0: RESERVIERT (Linux Kernel + NIC)")
        print()
        print("  [DRY-RUN] Prozesse werden NICHT gestartet.")
        print("  SharedMemory: nicht allokiert.")
        print("  Numba-JIT: nicht ausgeführt.")
        print()
        # Struct-Size Verify
        import ctypes
        class S4SharedState(ctypes.Structure):
            _fields_ = [
                ('current_volume',     ctypes.c_double),
                ('volume_ema',         ctypes.c_double),
                ('spike_multiplier',   ctypes.c_double),
                ('ob_depth_levels',    ctypes.c_int32),
                ('s4_pass_flag',       ctypes.c_int32),
                ('s4_label_code',      ctypes.c_int32),
                ('tick_counter',       ctypes.c_int32),
                ('timestamp_ns',       ctypes.c_double),
                ('validation_gate_ok', ctypes.c_int32),
                ('_pad',               ctypes.c_int32),
                ('_reserved',          ctypes.c_double),
            ]
        size = ctypes.sizeof(S4SharedState)
        print(f"  S4SharedState size: {size} bytes (erwartet: {SHM_SIZE}) — {'✓' if size == SHM_SIZE else '✗ FEHLER'}")
        print(f"  S4-Gate Konstanten: OB_THRESH={OB_DEPTH_THRESHOLD}, VOL_REQ={VOLUME_SPIKE_REQ}x, EMA_ALPHA={EMA_ALPHA:.6f}")
        print(f"  SHM_NAME: '{SHM_NAME}' ({SHM_SIZE} bytes)")
        print("=" * 60)
        print("  DRY-RUN BEENDET. Keine Prozesse gestartet.")
        print("=" * 60)
    else:
        main(validation_gate_status=args.gate)

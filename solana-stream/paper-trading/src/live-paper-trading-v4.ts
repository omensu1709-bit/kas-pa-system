...
class PaperTradingV4 {
  ...
  openPosition(symbol: string, amount: number, entryPrice: number): boolean {
    ...
    // KRITISCH: Prüfe auf Duplikate - nie denselben Coin zweimal!
    const existingIndex = this.positions.findIndex(p => p.symbol === symbol);
    if (existingIndex !== -1) {
      console.log(`[PaperTrading] ❌ DUPLICATE REJECTED: ${symbol} already has open position`);
      return false;
    }
...
class KasPAV4 {
  ...
  private readonly WARM_UP_DURATION_MS = 2 * 60 * 1000; // 2 minutes (FÜR IMMER!)
  ...
  private async runMainLoop(): Promise<void> {
    ...
    while (this.isRunning) {
      try {
        ...
        // SOTA v4.2: Start Propagation Tracking
        const traceId = propagationTracker.startTrace();
        let traceSuccess = true;
        let traceError = '';

        // 1. Update network data for crash detection
        propagationTracker.enterNode('CrashDetector', 'fetchNetworkData');
        try {
          await this.crashDetector.updateWithNetworkData();
          propagationTracker.exitNode('CrashDetector', true);
        } catch (e) {
          traceSuccess = false;
          traceError = (e as Error).message;
          propagationTracker.exitNode('CrashDetector', false);
          console.error('[Trace] CrashDetector error:', e);
        }
  ...
  // SOTA v5.0: Apply consensus filter to decision
        ...
        // SOTA v4.2: End trace FIRST to get telemetry data
        const traceResult = propagationTracker.endTrace(traceSuccess);
        ...
        // SOTA v4.2: Log trace result every 10 cycles
        if (traceResult && iteration % 10 === 0) {
          console.log(`[Trace] ID=${traceResult.traceId.substring(0, 12)}... | Propagation: ${traceResult.totalTimeMs.toFixed(1)}ms | Nodes: ${traceResult.nodeBreakdown.length}`);
        }
  ...
  private broadcastUpdate(stats: any, botMetrics: any): void {
    ...
    const update = {
      ...
      // SOTA v4.2: Telemetry Data (Propagation Tracking)
      telemetry: (() => {
        const lastTrace = propagationTracker.getLastTrace();
        return {
          traceId: lastTrace?.traceId || null,
          propagationTimeMs: lastTrace?.totalTimeMs || 0,
          nodeLatencies: lastTrace ? Object.fromEntries(
            lastTrace.nodeBreakdown.map(n => [n.nodeId, { avgLatencyMs: n.latencyMs, minLatencyMs: n.latencyMs, maxLatencyMs: n.latencyMs, sampleCount: 1, lastUpdate: Date.now() }])
          ) : {},
          lastTrace: lastTrace,
          timestamp: Date.now()
        };

...
import { consensusEngine } from './signals/multi-signal-consensus.js';
import { getOrderBookBoost } from './signals/orderbook-signal.js';
...
export class BayesianDecisionEngine {
  ...
  makeDecision(
    ...
    velocityBoost: number = 0,
    orderBookBoost: number = 0
  ): DecisionResult {
    ...
    const vBoost = (crashSignal as any).velocityBoost ?? velocityBoost ?? 0;
    const obSignal = (crashSignal as any).orderBookSignal;
    const obBoost = obSignal ? getOrderBookBoost(obSignal) : (orderBookBoost ?? 0);

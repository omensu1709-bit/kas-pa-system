...
export class InvestorAPI {
  ...
  constructor(engine: PaperTradingEngine, port: number = 8081) {
    this.engine = engine;
    this.port = port;
  }
   ...
   */
  private handlePerformance(res: http.ServerResponse): void {
    const perf = this.engine.getPerformance();
   ...
   * Offene Positionen
   */
  private handlePositions(res: http.ServerResponse): void {
    const perf = this.engine.getPerformance();
   ...
   * Trade Historie
   */
  private handleTrades(res: http.ServerResponse, url: URL): void {

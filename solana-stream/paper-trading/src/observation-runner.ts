...
interface ObservationMetrics {
  ...
  slot: number | null;
  dataFreshness: number; // ms since last successful fetch
  rpcLatency: number; // ms for RPC call
  wsClients: number;
...
async function chainstackRpc(method: string, params: any[] = []): Promise<any> {
  ...
  try {
    const response = await axios.post(CHAINSTACK_RPC, {
      ...
      method,
      params
    }, { auth: AUTH as any, timeout: 10_000 });
...
async function run24hObservation(): Promise<void> {
  ...
  let failedFetches = 0;
  let totalErrors = 0;
  const dataFreshnessHistory: number[] = [];
  ...
  while (Date.now() < endTime) {
    ...
    const observation: ObservationMetrics = {
      timestamp: now,
      uptime,
      slot: currentSlot,
      dataFreshness,

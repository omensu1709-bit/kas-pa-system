...
// ============================================================================
// MULTI-RPC CONFIGURATION (FAILOVER)
// ============================================================================
...
async function getActiveRpc(): Promise<RpcEndpoint> {
  return RPC_ENDPOINTS[activeRpcIndex];
}

async function switchToNextRpc(): Promise<RpcEndpoint> {
...
export class MultiRpcBotDetector {
   ...
   */
  private parseTransaction(tx: any): BotActivity | null {
  ...
  private updateBotProbability(): void {
    ...
    if (this.metrics.jitoBundleCount > 0) prob += 0.30;
    if (this.metrics.jitoBundleCount > 5) prob += 0.20;
    if (this.metrics.jitoBundleCount > 20) prob += 0.25;

    // High priority
    if (this.metrics.veryHighPriorityTxCount > 0) prob += 0.15;
    if (this.metrics.veryHighPriorityTxCount > 10) prob += 0.15;

    // MEV
    if (this.metrics.mevTxCount > 0) prob += 0.10;
   ...
   */
  getRecentActivities(limit = 50): BotActivity[] {

...
export interface PredictionRecord {
  ...
  // Trade actions (if any)
  action?: 'OPEN_POSITION' | 'CLOSE_POSITION' | 'REJECTED' | 'HALTED';
  actionReason?: string;
  positionId?: string;
  positionSize?: number;
  
  // Market outcome (filled after event window)
  actualCrash?: boolean;
  actualDrop?: number;
...
export class PredictionLogger {
  ...
  private pendingVerifications: Map<string, PredictionRecord[]> = new Map();
  private crashWindowMs: number = 24 * 60 * 60 * 1000; // 24h window
  private hashChain: string[] = [];

  constructor(private operatorId: string = 'system') {
  ...
  logAction(
    predictionId: string,
    action: PredictionRecord['action'],
    reason?: string,
    positionId?: string,
    positionSize?: number
  ): void {
   ...
   */
  async verifyPendingPredictions(): Promise<void> {
    const now = Date.now();
    const toVerify: PredictionRecord[] = [];

    for (const prediction of this.predictions) {
      if (prediction.verificationStatus !== 'PENDING') continue;
   ...
   */
  verifyIntegrity(): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  filter(criteria: {
    ...
    return this.predictions.filter(p => {
      if (criteria.zone && !criteria.zone.includes(p.zone)) return false;
      if (criteria.token && p.token !== criteria.token) return false;
      if (criteria.minProbability !== undefined && p.crashProbability < criteria.minProbability) return false;
      if (criteria.maxProbability !== undefined && p.crashProbability > criteria.maxProbability) return false;
      if (criteria.startTime && p.timestamp < criteria.startTime) return false;
      if (criteria.endTime && p.timestamp > criteria.endTime) return false;
      if (criteria.verifiedOnly && p.verificationStatus === 'PENDING') return false;
      return true;
    });
  }

  private calculateRecordHash(record: PredictionRecord): string {
    const content = {
      id: record.id,
  ...
  private addToPendingVerification(record: PredictionRecord): void {
  ...
  getHighProbabilitySignals(threshold: number = 0.2): PredictionRecord[] {

...
async function rpcRequest(method: string, params: any[] = []): Promise<any> {
  ...
  try {
    ...
    }, { timeout: 5000, auth: rpc.auth as any });
    rpc.failures = 0;
...
export class ComprehensiveBotDetector {
  ...
  private walletAge: Map<string, number> = new Map();
  private walletTxCount: Map<string, number> = new Map();
  private slotWallets: Map<number, Set<string>> = new Map();
  private metrics: BotMetrics;
  private isRunning = false;
  private pollInterval = 5000;
  private maxActivities = 2000;

  constructor() {
    this.metrics = this.initMetrics();
  }

  private initMetrics(): BotMetrics {
    return {
      jitoBundleCount: 0,
      sandwichCount: 0,
      arbitrageCount: 0,
   ...
   */
  private parseTransaction(tx: any, slot: number): BotActivity | null {
   ...
   * Detect Jito tip
   */
  private detectJitoTip(tx: any): number | null {
   ...
   */
  private extractComputePrice(tx: any): number {
  ...
  private detectBotType(
    ...
    for (const program of programs) {
      if (LENDING_PROGRAMS.has(program)) {
        const hasLiquidationIx = instructions.some((ix: any) => {
          const data = ix.data || '';
   ...
   */
  private extractPrograms(instructions: any[]): string[] {
   ...
   */
  private async scanJitoAccounts(): Promise<void> {
   ...
   */
  private async scanRecentBlocks(): Promise<void> {
    try {
   ...
   */
  private recordActivity(activity: BotActivity): void {
    // Deduplicate
    ...
    switch (activity.type) {
      case 'JITO_BUNDLE': this.metrics.jitoBundleCount++; break;
      case 'SANDWICH_BOT': this.metrics.sandwichCount++; break;
      case 'ARBITRAGE_BOT': this.metrics.arbitrageCount++; break;
      case 'LIQUIDATION_BOT': this.metrics.liquidationCount++; break;
      case 'SNIPER_BOT': this.metrics.sniperCount++; break;
      case 'BUNDLER_BOT': this.metrics.bundlerCount++; break;
      case 'VOLUME_BOT': this.metrics.volumeCount++; break;
      case 'COPY_TRADING_BOT': this.metrics.copyTradingCount++; break;
      case 'BACKRUN_BOT': this.metrics.backrunCount++; break;
    }
    ...
    if (activity.fee > 10000) this.metrics.highPriorityTxCount++;
    if (activity.tipLamports && activity.tipLamports > 500000) this.metrics.veryHighPriorityTxCount++;
   ...
   */
  private updateBotProbability(): void {
    let prob = 0;

    // Jito bundles (strongest signal)
    if (this.metrics.jitoBundleCount > 0) prob += 0.25;
    if (this.metrics.jitoBundleCount > 10) prob += 0.20;
    if (this.metrics.jitoBundleCount > 50) prob += 0.25;

    // Specific bot types
    if (this.metrics.sandwichCount > 0) prob += 0.15;
    if (this.metrics.liquidationCount > 0) prob += 0.20;
    if (this.metrics.sniperCount > 0) prob += 0.10;
    if (this.metrics.arbitrageCount > 0) prob += 0.10;
    if (this.metrics.backrunCount > 0) prob += 0.10;

    // High priority transactions
    if (this.metrics.veryHighPriorityTxCount > 0) prob += 0.15;
    ...
    for (const [, firstSlot] of this.walletAge) {
      if (typeof firstSlot === 'number' && firstSlot < 1000) ephemeral++;
    }
    this.metrics.ephemeralWallets = ephemeral;
    if (ephemeral > 5) prob += 0.20;
    if (ephemeral > 20) prob += 0.15;
   ...
   */
  getRecentActivities(limit = 100): BotActivity[] {
   ...
   * Get top bot wallets
   */
  getTopBotWallets(limit = 10): Array<{ wallet: string; count: number; type: BotType }> {
    const walletStats: Map<string, { count: number; type: BotType }> = new Map();

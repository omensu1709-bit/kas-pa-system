...
export interface EpidemicParams {
  ...
  decayRate: number;        // Exponential decay rate for old infections
  threshold: number;        // Cascade size threshold to count as "infection"
}

export interface EpidemicResult {
  rt: number;               // R_t reproduction number
  isSupercritical: boolean; // R_t > 1
  metadata: {
...
export class EpidemicMetric {
  private infections: InfectionEvent[] = [];
  private params: EpidemicParams;
   ...
   */
  addTransmission(sourceSlot: number, targetSlot: number, weight: number = 1): void {
    if (targetSlot <= sourceSlot) return; // Invalid: target must be after source
  ...
  private emptyResult(): EpidemicResult {
    return {
      rt: 0,

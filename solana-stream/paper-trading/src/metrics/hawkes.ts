...
export interface HawkesParams {
  decay: number;          // Decay parameter λ
  maxLag: number;          // Maximum lag to consider (slots)
  windowSize: number;      // Rolling window size
}

export interface HawkesResult {
  branchingRatio: number;  // n ∈ [0, ∞), n→1 = critical
...
export class HawkesMetric {
  private events: number[] = [];  // timestamps in slot numbers
  ...
  private emptyResult(): HawkesResult {
    return {
      branchingRatio: 0,
      intensity: 0,
      metadata: {

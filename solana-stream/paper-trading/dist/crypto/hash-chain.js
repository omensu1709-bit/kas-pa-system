...
export class HashChain {
    ...
    createEntry(data) {
        ...
        const entryHash = CryptoJS.SHA256(`${entryId}:${contentHash}:${previousHash}:${timestamp}`).toString();
        const entry = {
            id: entryId,
            timestamp,
            previousHash,
    ...
    verify() {
        ...
        for (let i = 0; i < this.chain.length; i++) {
            ...
            const expectedEntryHash = CryptoJS.SHA256(`${entry.id}:${entry.contentHash}:${entry.previousHash}:${entry.timestamp}`).toString();

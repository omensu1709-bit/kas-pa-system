...
export interface Prediction {
  ...
  consensus: number;        // 0-1, consensus score
  expectedDrop: number;     // e.g. 0.035 = 3.5%
  timeframe: number;        // minutes
  positionSize: number;     // SOL (for paper trading)
  layersAgreeing: number;   // how many layers support this decision
  reasons: string[];        // human-readable reasons
}

export interface ConsensusResult {
  consensus: number;           // 0-1, weighted consensus score
  layers: SignalLayer[];       // individual layer data
  layersAgreeing: number;      // count of layers with score > 0.6
  highSignalLayers: string[];   // names of layers with high signals
}
...
export function makeDecision(
  ...
  if (marketLayer.signals.sellAcceleration > 0.6) reasons.push(`Sell acceleration ${(marketLayer.signals.sellAcceleration * 100).toFixed(0)}%`);
  if (marketLayer.signals.buySellImbalance > 0.6) reasons.push(`${(marketLayer.signals.buySellImbalance * 100).toFixed(0)}% sells`);
  if (volumeDiv > 0.5) reasons.push(`Volume-price divergence`);
  if (whalePressure > 0.3) reasons.push(`Whale sell pressure ${(whalePressure * 100).toFixed(0)}%`);
  if (onChainLayer.signals.uniqueSellerSurge > 0.5) reasons.push(`Mass seller exodus`);
  
  // HIGH CONFIDENCE SHORT (3 layers agree OR 2 layers + strong signals)
  if (
    (result.consensus > 0.80 && sellAccel > 0.75 && whalePressure > 0.40) ||
...
export function assessSeverity(
  marketLayer: MarketLayer,
  onChainLayer: OnChainLayer
): { severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'; score: number } {
  let score = 0;
  
  // Market signals
  if (marketLayer.signals.sellAcceleration > 0.8) score += 0.25;
  else if (marketLayer.signals.sellAcceleration > 0.6) score += 0.15;
  
  if (marketLayer.signals.volumePriceDivergence > 0.8) score += 0.25;
  else if (marketLayer.signals.volumePriceDivergence > 0.5) score += 0.10;
  
  if (marketLayer.signals.buySellImbalance > 0.70) score += 0.15;
  
  if (marketLayer.signals.liquidityDrainage > 0.6) score += 0.20; // Rug pull risk
  
  // OnChain signals
  if (onChainLayer.signals.whaleSellPressure > 0.5) score += 0.20;
  if (onChainLayer.signals.uniqueSellerSurge > 0.7) score += 0.15;
  
  // Classify severity
  let severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  if (score >= 0.75) severity = 'CRITICAL';
  else if (score >= 0.55) severity = 'HIGH';

...
export interface MarketSignals {
  sellAcceleration: number;    // 0-1, higher = more acceleration
  buySellImbalance: number;    // 0-1, 0.6 = 60% sells
  volumePriceDivergence: number; // 0-1, higher = distribution pattern
  liquidityDrainage: number;    // 0-1, higher = more drainage
}

export interface MarketLayer {
  score: number;                // 0-1, overall layer score
 ...
 */
export function calculateSellAcceleration(data: { txns: { m5: { sells: number }; h1: { sells: number } } }): number {
 ...
 */
export function calculateBuySellImbalance(data: { txns: { m5: { buys: number; sells: number } } }): number {
...
export function calculateVolumePriceDivergence(data: {
  ...
  if (volumeSpike > 2.5 && priceChange < -2.0) return 1.0;
  if (volumeSpike > 2.0 && priceChange < -1.5) return 0.85;
  if (volumeSpike > 2.0 && priceChange < -1.0) return 0.7;
  if (volumeSpike > 1.5 && priceChange < -0.5) return 0.4;
  
  // Mild divergence
  if (volumeSpike > 1.5 && priceChange < 0) return 0.2;
 ...
 */
export function calculateLiquidityDrainage(mint: string, currentLiquidity: number): number {
  const now = Date.now();
  ...
  if (change >= 0) return 0; // No drainage
  if (change < -0.20) return 1.0; // Cap at -20%
 ...
 */
export async function calculateMarketLayer(
  dexData: { txns: { m5: { buys: number; sells: number }; h1: { buys: number; sells: number } }; volume: { m5: number; h1: number }; priceChange: { m5: number }; liquidity: { usd: number } },
  mint: string
): Promise<MarketLayer> {
  ...
  const score = (
    signals.sellAcceleration * 0.35 +      // 35% - most important
  ...
  const confidence = Math.min(1.0, (
    (dexData.txns.m5.buys + dexData.txns.m5.sells > 10 ? 0.3 : 0) +
 ...
 */
export function hasSevereMarketSignal(layer: MarketLayer): boolean {
  return (
    layer.signals.sellAcceleration > 0.8 ||

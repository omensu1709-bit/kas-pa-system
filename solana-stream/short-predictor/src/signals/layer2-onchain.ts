...
export interface OnChainSignals {
  whaleSellPressure: number;  // 0-1, higher = more whale selling
  uniqueSellerSurge: number;  // 0-1, higher = more unique sellers
}
 ...
 */
export function calculateWhaleSellPressure(txs: EnhancedTransaction[], mint: string): number {
 ...
 */
export function calculateUniqueSellerSurge(txs: EnhancedTransaction[], mint: string): number {
 ...
 */
export async function fetchHeliusTransactions(mint: string, limit: number = 20): Promise<EnhancedTransaction[]> {
  try {
    ...
    const txResponse = await axios.post(
      HELIUS_ENHANCED_URL,
      { transactions: signatures.slice(0, 10) }, // Limit to 10 for efficiency
      { timeout: 15000 }
    );
 ...
 */
export async function calculateOnChainLayer(
  mint: string,
  txs?: EnhancedTransaction[]
): Promise<OnChainLayer> {
  ...
  const score = (
    signals.whaleSellPressure * 0.60 +    // 60% - primary indicator
 ...
 */
export function hasSevereOnChainSignal(layer: OnChainLayer): boolean {
  return (
    layer.signals.whaleSellPressure > 0.5 ||

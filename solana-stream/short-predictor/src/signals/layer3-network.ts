...
export interface NetworkSignals {
  txVolumeVerification: number;  // 0-1, higher = more TX activity
}
 ...
 */
export async function getRecentTxCount(mint: string, lookbackSlots: number = 10000): Promise<number> {
  try {
 ...
 */
export function calculateTxVolumeVerification(recentTxCount: number): number {
 ...
 */
export async function detectFeeSpike(mint: string): Promise<number> {
  try {
    // Get recent signatures
 ...
 */
export async function calculateNetworkLayer(mint: string): Promise<NetworkLayer> {
  // Fetch network data
  ...
  const score = (
    signals.txVolumeVerification * 0.70 +
    feeSpike * 0.30
  );
 ...
 */
export async function checkChainstackHealth(): Promise<{ healthy: boolean; latency: number }> {
 ...
 */
export function hasSevereNetworkSignal(layer: NetworkLayer): boolean {

...
export class ChainstackClient {
  ...
  private config: ChainstackClientConfig;
  ...
  async start(tokenMints: string[]): Promise<void> {
    ...
    try {
      ...
      this.stream.on('data', async (data: any) => {
        ...
        if (data.account) {
          ...
          if (pubkey && !this.deduplication.isAccountDuplicate(
            pubkey,
            data.slot,
  ...
  async updateSubscriptions(tokenMints: string[]): Promise<void> {

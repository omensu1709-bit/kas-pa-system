...
export interface SeismicParams {
  windowSize: number;       // Number of events
  minMagnitude: number;     // Minimum "magnitude" to consider
  bins: number;             // Number of magnitude bins
}

export interface SeismicResult {
  bValue: number;           // Gutenberg-Richter b
  isStressed: boolean;      // b < 1.0
  metadata: {
    totalEvents: number;
    meanMagnitude: number;
    maxMagnitude: number;
    aValue: number;         // Activity rate
  };
}
...
export class GutenbergRichterMetric {
  ...
  addMagnitude(magnitude: number): void {
    if (magnitude < this.params.minMagnitude) return;
  ...
  private emptyResult(): SeismicResult {
    return {
      bValue: 0,
      isStressed: false,
      metadata: {
